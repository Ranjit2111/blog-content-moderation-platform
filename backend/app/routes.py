...
from app.schemas import PostCreate, PostResponse, ModerationResponse
...
async def submit_post(post_id: int, db: AsyncSession = Depends(get_db)):
    ...
    status, reasons = moderate_content(post.content)
    ...
    return ModerationResponse(status=status, reasons=reasons if status == "flagged" else None)
